...
# Store last page for Profile back button
...
with main_col:
    ...
    with c1:
        ...
        render_chart(df, "pie", "severity", title="Severity Breakdown")

    with c2:
        st.markdown("### 🧩 Category")
        render_chart(df, "bar", "category", color="severity",
                     title="Incidents by Category")

    with c3:
        st.markdown("### 📈 Trend")
        render_chart(df, "line", "timestamp", title="Incident Trend")
    ...
    with st.form("add_incident"):
        ...
        if st.form_submit_button("Add"):
            insert_incident(datetime.now().isoformat(), sev, cat, status, desc)
            reload_page()

with ai_col:
    st.subheader("🧠 Cyber AI")

    history = load_ai_history(username, "cyber") or []
    for msg in history:
    ...
    if st.button("Send") and ai_input:
        save_ai_message(username, "cyber", "user", ai_input)
        ...
        if get_gemini_response:
            ...
            save_ai_message(username, "cyber", "assistant", full)

        reload_page()

    if st.button("🧹 Clear Chat History"):
        delete_ai_history(username, "cyber")
        reload_page()
